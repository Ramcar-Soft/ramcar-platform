...
- Q: On every API path that mutates `user_tenants` (admin auto-assignment on tenant create, multi-tenant user create, multi-tenant user update/delete), should the same transaction also call `supabase.auth.admin.updateUserById` to synchronize `auth.users.raw_app_meta_data.tenant_ids` — in addition to maintaining the `user_tenants` rows? → A: Yes — mirror `user_tenants` into `raw_app_meta_data.tenant_ids` on every mutation; the `custom_access_token_hook` remains authoritative at token issue.
...
- **FR-028a**: Every API code path that mutates `user_tenants` MUST also call `supabase.auth.admin.updateUserById` for each affected user in the same request, writing the updated `tenant_ids` array to `raw_app_meta_data.tenant_ids`. This mirrors the join table into the user record so the two sources stay in lockstep; the `custom_access_token_hook` (FR-024) remains authoritative at token issue. The affected paths at minimum are: (a) admin auto-assignment inside `CreateTenantUseCase` (updates the creating admin only), (b) multi-tenant user create in `UsersService.create` for `admin`/`guard` roles (updates the created user), (c) multi-tenant user update in `UsersService.update` for `admin`/`guard` roles whenever the selected tenant set changes (updates the edited user), (d) any future path that directly inserts/deletes `user_tenants` rows. For super_admin users the written value MUST be the wildcard `"*"`; for residents the written value MUST be a one-element array matching `profiles.tenant_id`. If the `auth.admin.updateUserById` call fails after a successful `user_tenants` write, the request MUST surface the failure (5xx) so the caller retries; silent drift between `user_tenants` and `raw_app_meta_data` is not acceptable.
...
- **Token refresh after Admin creates a new tenant**: The Admin's new `user_tenants` row is not visible in their JWT until the token is refreshed. The client library (`@supabase/ssr` or the desktop auth client) handles refresh on its normal schedule (short-lived access tokens). The spec does not require a forced refresh; the TopBar selector picks up the new tenant at most one token-TTL later. Per FR-028a, the API ALSO writes the updated `tenant_ids` set to `auth.users.raw_app_meta_data.tenant_ids` in the same request so the user record stays in lockstep with `user_tenants`, but this is defense-in-depth — the JWT's `tenant_ids` claim is still minted by the `custom_access_token_hook` at the next token refresh. Optional UX: show an info toast after create ("This tenant will appear in your selector shortly") — planning decision.
