...
- **Shared packages**: Zod schema reuse between API validation and frontend forms, `@ramcar/` scope conventions, no manual edits to `packages/db-types`, `@ramcar/i18n` as single source for bi-app strings
- **Cross-app shared feature modules (CLAUDE.md § "Cross-App Shared Feature Modules", spec 014)**: Bi-app feature code (today: `visitors`, `residents`, `providers`) lives in the shared feature-modules workspace package, not in per-app `src/features/`. Shared modules MUST NOT import `next/*`, MUST NOT use `"use client";`, MUST NOT hardcode `next-intl`/`react-i18next`, MUST NOT hardcode an online-only transport. Deliberate platform extensions (web `useFormPersistence`, desktop offline/sync badge, admin-only actions) are host-supplied extension points, not forks.
- Anti-patterns: `any` types, unscoped queries (missing tenant_id), direct cross-feature imports, business logic in routing/IPC layers, direct SQLite access outside repositories, React Query/Zustand overlap, **duplicated bi-app feature components under both `apps/web/src/features/X/` and `apps/desktop/src/features/X/` instead of the shared feature-modules package**, **shared feature modules importing framework-specific APIs (`next/*`, `window.electron`, `"use client";`)**, **locale strings for bi-app features duplicated across per-app message files instead of `@ramcar/i18n`**.
